...
function Screen() {
  ...
  return (
    <div style={{ ...STYLES.SCREEN, backgroundImage: `url(${backgroundByTime.current})` }}>
      ...
      {!state.running && !state.gameover && <img style={{ ...STYLES.INSTRUCTION }} src={instructionImg} alt="instruction" />}
      <Bird {...state} dispatch={dispatch} />
      <Pipe {...state} initX={state.pipe.initX} height={randomHeightPipe(SCREEN_HEIGHT, MAX_PIPE_HEIGHT_PERCENT)} dispatch={dispatch} />
